...
def building_training_dataset(session_positive, session_negative):
    ...
    for i in range(len(session_positive)):
        ...
        for j in range(len_pos):
            if len_neg < hparams['gnn_neg_ratio']:
                nega_sample = []
                for s in range(int(hparams['gnn_neg_ratio'] / len_neg)):
                    nega_sample.append(neg_link)
                neg_sample_id = np.random.choice(len_neg, hparams['gnn_neg_ratio'] % len_neg, replace=False)
                nega_sample.append(neg_link[neg_sample_id])
                nega_sample = np.concatenate(nega_sample)
            else:
                neg_sample_id = np.random.choice(len_neg, hparams['gnn_neg_ratio'], replace=False)
...
def node2vec(mind_dgl, save_dir, force_reload=False):
    try:
        if force_reload:
            raise Exception('Force Reload.')
        node2vec = torch.load('{}/node2vec.pth'.format(save_dir))
        print('load from files')
    except Exception as e:
